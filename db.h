...
extern int cfg_wallet_port[SEEDER_COUNT];
extern std::string sAppName;
extern std::string cfg_explorer_url[SEEDER_COUNT];
...
public:
  ...
  bool IsGood() const {
    if (ip.GetPort() != cfg_wallet_port[TempMainThreadNumber]) return false;
    if (!(services & NODE_NETWORK)) return false;
    if (!ip.IsRoutable()) return false;
    if (clientVersion && clientVersion < cfg_min_peer_proto_version[TempMainThreadNumber]) return false;
  ...
  int GetBanTime() const {
    if (clientVersion && clientVersion < cfg_min_peer_proto_version[TempMainThreadNumber]) { return 604800; }
    if (IsGood()) return 0;
  ...
  IMPLEMENT_SERIALIZE (({
    ...
    SHARED_CRITICAL_BLOCK(cs) {
      if (fWrite) {
		...
		//printf("ourId.size() + unkId.size():%d %d \n", (int)ourId.size(), (int)unkId.size());
        READWRITE(n);
        for (std::deque<int>::const_iterator it = ourId.begin(); it != ourId.end(); it++) {
